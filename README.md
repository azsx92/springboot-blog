...
- Port 8080 was already in use.Action
  포트 8080이 이미 다른 프로세스에서 사용 중이라서 발생한 오류입니다. 이 경우, 해당 프로세스를 종료하거나 다른 포트를 사용하도록 설정할 수 있습니다.

### 포트 8080을 사용하는 프로세스 확인 및 종료 (Mac)

1. **포트 8080을 사용하는 프로세스 확인**
   터미널에서 다음 명령어를 입력하여 8080 포트를 사용하는 프로세스를 확인합니다:

   ```bash
   lsof -i :8080
   ```

   이 명령어는 8080 포트를 사용 중인 프로세스의 PID(Process ID)를 보여줍니다. 예시 출력은 아래와 같을 수 있습니다:

   ```
   COMMAND   PID  USER   FD   TYPE DEVICE SIZE/OFF NODE NAME
   java     1234  user   123u  IPv6 0x12345678      0t0  TCP *:http-alt (LISTEN)
   ```

2. **프로세스 종료**
   포트를 사용하는 프로세스의 PID를 찾은 후, `kill` 명령어로 해당 프로세스를 종료할 수 있습니다. 예를 들어 PID가 `1234`라면:

   ```bash
   kill -9 1234
   ```

   이 명령어는 PID가 `1234`인 프로세스를 강제로 종료시킵니다.

### 다른 포트 사용하도록 설정

1. **application.properties 또는 application.yml 수정**
   Spring Boot 애플리케이션에서 다른 포트를 사용하려면, `application.properties` 또는 `application.yml` 파일을 수정해야 합니다.

   `application.properties`에서 포트를 변경하려면:

   ```properties
   server.port=8081  # 예시로 8081 포트를 사용
   ```

   `application.yml`에서 포트를 변경하려면:

   ```yaml
   server:
     port: 8081  # 예시로 8081 포트를 사용
   ```

이렇게 하면 애플리케이션이 다른 포트에서 실행됩니다.

포트를 변경하는 방법이 더 편리할 수 있지만, 우선 포트를 차지하고 있는 프로세스를 종료하면 원래 포트인 8080을 사용할 수 있습니다.

### 여기서 또 궁금한 점 Kill 앞에 -9 무엇을 뜻하는걸까?
`kill -9 1234`에서 `-9`는 운영체제에 특정 프로세스(PID 1234)를 **즉시 종료**하라는 신호를 의미합니다. 구체적으로, `-9`는 **SIGKILL** 신호를 나타냅니다.

### SIGKILL (신호 9) 설명:
- **SIGKILL**은 운영체제에게 프로세스를 **강제로 종료**하라는 명령을 내립니다. 다른 신호들과 달리 **SIGKILL은 프로세스가 이를 차단하거나 무시할 수 없습니다.** 따라서 매우 강력한 종료 방법입니다.
- 프로세스가 종료되기 전에 리소스를 정리하거나 작업을 마칠 시간을 주지 않기 때문에, 즉시 종료됩니다.

### 왜 `-9`를 사용하나요?
- **프로세스가 정상적으로 종료되지 않을 때** 일반적으로 `kill` 명령으로 **SIGTERM** 신호(기본적으로 `kill <PID>`)를 보내 종료를 요청합니다. 그러나 프로세스가 이에 응답하지 않으면, `kill -9`를 사용하여 **강제로 프로세스를 종료**할 수 있습니다.
- `-9` 신호는 **프로세스가 응답하지 않거나 멈춰버린 상태에서 강제로 종료할 때 사용**됩니다.

### 요약:
- `kill 1234`: **SIGTERM** 신호를 보내 프로세스에 정상적인 종료를 요청합니다.
- `kill -9 1234`: **SIGKILL** 신호를 보내 프로세스를 즉시 강제로 종료합니다.

`-9`를 사용할 때는 **프로세스가 리소스를 정리하거나 파일을 닫을 시간이 주어지지 않기 때문에, 데이터를 손상시킬 수 있습니다.** 따라서 `-9`는 다른 방법이 통하지 않을 때 마지막 수단으로 사용해야 합니다.


### 왜 final를 사용하는 걸까?
`final` 키워드를 사용하는 이유는 클래스의 **불변성(immutability)**을 보장하기 위해서입니다. 간단히 말해, 한 번 값이 설정되면 그 값을 변경할 수 없도록 만들기 위함입니다.

### `final`을 사용하는 이유:

1. **불변 객체 만들기**:
    - `final`로 선언된 필드는 초기화 이후 값을 변경할 수 없습니다. 즉, 객체가 생성될 때 `id`, `title`, `content` 값이 설정되면, 그 후에 다른 값으로 변경할 수 없다는 뜻입니다.
    - 이렇게 불변 객체를 만들면, 객체가 변경되지 않아서 **예측 가능하고 안정적**한 코드가 됩니다. 특히 멀티쓰레드 환경에서 여러 쓰레드가 객체를 동시에 참조하더라도, 값을 변경할 걱정 없이 안전하게 사용할 수 있습니다.

2. **설계 의도 표현**:
    - `final`을 사용함으로써, 해당 클래스가 **불변성**을 갖도록 설계했다는 의도를 명확히 표현할 수 있습니다. 이는 코드의 가독성을 높이고, 다른 개발자가 해당 필드들이 변경되지 않음을 쉽게 알 수 있게 해줍니다.

3. **일관성 유지**:
    - 필드를 `final`로 선언하면, 객체를 생성한 후에 필드 값이 **변경되지 않음을 보장**할 수 있습니다. 예를 들어, 어떤 코드에서 이 객체를 사용할 때, 그 객체의 필드가 변하지 않음을 확신할 수 있어 코드가 더 안정적이고 예측 가능해집니다.

### `private Long id`만 사용하는 경우:
- 만약 `final`을 제거하고 `private Long id`처럼 선언한다면, 이후에 다른 코드에서 해당 값을 변경할 수 있게 됩니다. 예를 들어, setter 메소드나 다른 방법으로 값을 변경할 수 있어, **불변성을 보장할 수 없게 됩니다**.
- 객체가 생성된 후에 값이 바뀌는 경우가 생기면, 그 객체의 상태가 예기치 않게 변경될 수 있기 때문에 **디버깅이 어려워지고 오류가 발생할 가능성**이 높아질 수 있습니다.

### 결론:
`final`을 사용하는 이유는 **불변 객체를 만들기 위해서**입니다. 이렇게 불변 객체로 설계하면 코드의 안정성과 예측 가능성을 높일 수 있습니다. 만약 객체의 상태가 변하지 않아야 한다면, `final`을 사용하는 것이 좋은 방법입니다.

### stream에서 나는 :: new 가 의미하는게 무엇인지 궁금했다.

여기서 `ArticleListViewResponse::new`는 **생성자 참조(constructor reference)**라는 문법입니다. 쉽게 말해, **`new ArticleListViewResponse(article)`을 람다식 없이 간결하게 표현한 것**이에요.
...
이 부분은 다음과 같은 람다식과 동일합니다:

```java
.map(article -> new ArticleListViewResponse(article))
```

즉, `findAll()`로 가져온 `Article` 객체들을 하나씩 꺼내서, 그걸 가지고 `ArticleListViewResponse` 객체로 변환하는 역할을 합니다.

---

### 💡 `::new`가 하는 일

- `ArticleListViewResponse` 클래스에 `Article`을 파라미터로 받는 생성자가 존재하니까,
- 스트림에서 `map()`을 돌릴 때 각 `Article`을 `new ArticleListViewResponse(article)`로 변환하고,
- 이걸 생성자 참조 문법인 `ArticleListViewResponse::new`로 축약해서 표현한 거예요.

---

### ✅ 요약

- `::new`는 **생성자 참조**를 의미합니다.
